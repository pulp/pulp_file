...
if [[ "${RELEASE_WORKFLOW:-false}" == "true" ]]; then
  REPORTED_VERSION="$(echo $REPORTED_STATUS | jq --arg plugin file --arg legacy_plugin pulp_file -r '.versions[] | select(.component == $plugin or .component == $legacy_plugin) | .version')"
...
pushd ../pulp-cli
pip install -r test_requirements.txt pytest-custom_exit_code
pytest --suppress-no-test-exit-code -v -m pulp_file
popd
