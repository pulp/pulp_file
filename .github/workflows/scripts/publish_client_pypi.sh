...
export VERSION=$(ls dist | sed -rn 's/pulp_file-client-(.*)\.tar.gz/\1/p')
...
twine upload dist/pulp_file_client-$VERSION-py3-none-any.whl -u pulp -p $PYPI_PASSWORD
twine upload dist/pulp_file-client-$VERSION.tar.gz -u pulp -p $PYPI_PASSWORD
