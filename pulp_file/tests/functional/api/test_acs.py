...
class AlternateContentSourceTestCase(unittest.TestCase):
    ...
    def test_acs_sync(self):
        ...
        # the sync show now work as the files are being pulled from ACS remote
        sync_response = self.repo_api.sync(repo.pulp_href, repository_sync_data)
        monitor_task(sync_response.task)
    ...
    def test_serving_acs_content(self):
        ...
        self.addCleanup(self.publication_api.delete, publication_href)

        # Create a distribution.
        body = gen_distribution()
        body["publication"] = publication_href
        distribution_response = self.distribution_api.create(body)
        ...
        download_content_unit(cfg, distribution.to_dict(), unit_path)
